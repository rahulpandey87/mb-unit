...
namespace QuickGraph.UnitTests
{
	...
	/// incidence graph implemetation of <see cref="Vertex"/> and
	/// <see cref="Edge"/>.
	/// </summary>
	public class CustomAdjacencyGraph:
		IMutableGraph
		...
		,IMutableBidirectionalVertexAndEdgeListGraph
	{
		...
		private VertexEdgeCollectionDictionary vertexOutEdges = new VertexEdgeCollectionDictionary();
		private VertexEdgeCollectionDictionary vertexInEdges = new VertexEdgeCollectionDictionary();
		...
		public CustomAdjacencyGraph(bool allowParallelEdges)
			:this(
			new VertexProvider(),
			new EdgeProvider(),
			allowParallelEdges
		...
		#region IGraph
		/// <summary>
		/// Gets a value indicating if the <see cref="CustomAdjacencyGraph"/> 
		/// is directed.
		/// </summary>
		/// <value>
		...
		/// <summary>
		/// Gets a value indicating if the <see cref="CustomAdjacencyGraph"/> allows parallel edges.
		/// </summary>
		/// <value>
		/// true if the <see cref="CustomAdjacencyGraph"/> is a multi-graph, false otherwise
		/// </value>
		...
		/// <summary>
		/// Gets the <see cref="Vertex"/> provider
		/// </summary>
		/// <value>
		/// <see cref="Vertex"/> provider
		/// </value>
		public VertexProvider VertexProvider
		{
			get
			{
		...
		{
			this.version++;
			Vertex v = VertexProvider.ProvideVertex();
		...
		{
			return this.AddEdge((Vertex)source,(Vertex)target);
		}
		...
		{
			this.RemoveEdge((Vertex) u, (Vertex) v);
		}
		...
		{
			return this.OutEdgesEmpty((Vertex)v);
		}
		...
		/// <returns>number of out-edges of the <see cref="Vertex"/> v</returns>
		/// <exception cref="ArgumentNullException">
		/// v is a null reference (Nothing in Visual Basic)
		/// </exception>
		/// <exception cref="VertexNotFoundException">
		...
		{
			return this.OutDegree((Vertex)v);
		}
		...
		/// v is not part of the graph.
		/// </exception>
		public IEdgeCollection OutEdges(Vertex v)
		{
			if (v == null)
				throw new ArgumentNullException("v");
		...
		{
			return this.ContainsEdge((Vertex)u,(Vertex)v);
		}		
		#endregion 
		...
		{
			return this.SelectSingleOutEdge((Vertex)v,ep);
		}
		...
		{
			return this.SelectOutEdges((Vertex)v,ep);
		}
		...
		{
			this.RemoveOutEdgeIf((Vertex)u,pred);
		}
		#endregion
		...
		{
			return this.ContainsVertex((Vertex)v);
		}
		#endregion
		
		#region IFilteredVertexListGraph
		/// <summary>
		/// Returns the first <see cref="Vertex"/> that matches the predicate
		/// </summary>
		...
		public Vertex SelectSingleVertex(IVertexPredicate vp)
		{
			if (vp == null)
				throw new ArgumentNullException("vertex predicate");

			foreach(Vertex v in this.SelectVertices(vp))
				return v;
			return null;
		}
		...
		{
			return AdjacentVertices((Vertex)v);
		}
		#endregion
		...
		/// </exception>
		public bool InEdgesEmpty(Vertex v)
		{
			if (v==null)
				throw new ArgumentNullException("v");
		...
		{
			return this.InEdgesEmpty((Vertex)v);
		}
		...
		{
			return this.AdjacentEdgesEmpty((Vertex)v);
		}
		...
		/// matches the predicate.</returns>
		/// <exception cref="ArgumentNullException">v or ep is null</exception>
		public Edge SelectSingleInEdge(Vertex v, IEdgePredicate ep)
		{
			if (ep==null)
				throw new ArgumentNullException("edge predicate");
		...
		{
			return this.SelectSingleInEdge((Vertex)v, ep);
		}
		...
		/// criteron</returns>
		/// <exception cref="ArgumentNullException">v or ep is null</exception>
		public IEdgeEnumerable SelectInEdges(Vertex v, IEdgePredicate ep)
		{
			if (v==null)
				throw new ArgumentNullException("vertex");
			if (ep==null)
		...
		{
			return this.SelectInEdges((Vertex)v,ep);
		}
		#endregion
		...
		{
			this.RemoveInEdgeIf((Vertex)u,pred);
		}
		...
		#region VertexEdgeCollectionDictionary
		private class VertexEdgeCollectionDictionary :
			DictionaryBase
		{
			public VertexEdgeCollectionDictionary()
			{}
			...
			public IVertexEnumerable Vertices
			{
				get
				{
					return new VertexEdgeCollectionVertexEnumerable(this.Dictionary.Keys);
				}
			}
			...
			public IEdgeEnumerable Edges
			{
				get
				{
					return new VertexEdgeCollectionEdgeEnumerable(this.Dictionary.Values);
				}			
			}
			...
			private class VertexEdgeCollectionVertexEnumerable : 
				IVertexEnumerable
			{
				private IEnumerable en;
				public VertexEdgeCollectionVertexEnumerable(IEnumerable en)
				{
				...
				{
					return new VertexEdgeCollectionVertexEnumerator(en);
				}
				...
				#region Enumerator
				private class VertexEdgeCollectionVertexEnumerator :
					IVertexEnumerator
				{
					private IEnumerator en;
					public VertexEdgeCollectionVertexEnumerator(IEnumerable col)
					{
			...
			private class VertexEdgeCollectionEdgeEnumerable : 
				IEdgeEnumerable
			{
				private IEnumerable en;
				public VertexEdgeCollectionEdgeEnumerable(IEnumerable en)
				{
					Debug.Assert(en!=null);
					this.en = en;
				}
				...
				{
					return new VertexEdgeCollectionEdgeEnumerator(en);
				}
				IEnumerator IEnumerable.GetEnumerator()
				{
					return this.GetEnumerator();
				...
				private class VertexEdgeCollectionEdgeEnumerator :
					IEdgeEnumerator
				{
					private IEnumerator edges;
					private IEdgeEnumerator edge;

					public VertexEdgeCollectionEdgeEnumerator(IEnumerable en)
					{
						Debug.Assert(en!=null);
		...
		private class VertexEnumerator : IVertexEnumerator
		{
			private IEnumerator en;
			public VertexEnumerator(IEnumerable enumerable)
			{
				Debug.Assert(en!=null);
