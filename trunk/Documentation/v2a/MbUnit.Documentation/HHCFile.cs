...
namespace MbUnit.Documentation
{
    public class HHCFile
    {
        ...
        public static void MergeTOC(string rootFolder, string sourceFolder)
        {
            ...
            string[] projectTOCFiles = Directory.GetFiles(rootFolder, "*.hhc");
            ...
            foreach (string tocPath in projectTOCFiles)
            {
                ...
                if (newTOCString.Contains(APIFLAG))
                {
                    ...
                    mergedContent = string.Format("<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML/EN\"><HTML><BODY><UL>{0}</UL></BODY></HTML>", newContents);
                }
                else
                {
        ...
        private static string ProcessXmlNodes(XmlNodeList nodes)
        {
            ...
            foreach (XmlNode node in nodes)
            {
                ...
                if (node.HasChildNodes)
                {
                    ...
                    temptoc = temptoc.Insert(temptoc.Length, "</UL>");
                    temptoc = temptoc.Insert(0, "<UL>");
                }
